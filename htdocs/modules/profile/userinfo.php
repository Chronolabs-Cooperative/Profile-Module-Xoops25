...
if (is_object($GLOBALS['xoopsUser']) && $uid == $GLOBALS['xoopsUser']->getVar('uid')) {
    ...
    if (!is_object($thisUser) || (!$thisUser->isActive() && (!$GLOBALS['xoopsUser'] || !$GLOBALS['xoopsUser']->isAdmin() ))) {
        redirect_header(XOOPS_URL . "/modules/" . $GLOBALS['profileModule']->getVar('dirname', 'n'), 3, _US_SELECTNG);
        exit();
    }
    ...
    $gperm_handler =& xoops_gethandler('groupperm');
    $groups_accessible = $gperm_handler->getItemIds('profile_access', $groups_xoopsUser, $GLOBALS['profileModule']->getVar('mid'));
    ...
    if ($rejected) {
        redirect_header(XOOPS_URL . "/modules/" . $GLOBALS['profileModule']->getVar('dirname', 'n'), 3, _NOPERM);
        exit();
    }
